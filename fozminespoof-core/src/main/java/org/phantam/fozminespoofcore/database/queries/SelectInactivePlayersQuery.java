...
/**
 * Selects all inactive fake players (is_active = 0).
 */
public class SelectInactivePlayersQuery implements Query<Collection<FakePlayerData>> {

    private final String tableName;

    public SelectInactivePlayersQuery(String tableName) {
        this.tableName = tableName;
    }

    @Override
    public Collection<FakePlayerData> execute(Connection connection) throws SQLException {
        String sql = "SELECT * FROM " + tableName + " WHERE is_active = 0;";
