...
public final class NMSBridgeLoader {
     ...
     * @return a FozminespoofApi instance, or null if loading fails
     */
    public static FozminespoofApi loadBridge(Logger logger) {
        ...
        if (versionKey == null) {
            ...
            org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: unsupported version '%s'", rawVersion);
            return null;
        }

        String className = "org.phantam.fozminespoofv" + versionKey + ".NMSBridge_v" + versionKey;
        org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: attempting to load class '%s'", className);

        try {
            Class<?> clazz = Class.forName(className);
            Object instance = clazz.getDeclaredConstructor().newInstance();

            if (instance instanceof FozminespoofApi) {
                logger.log(Level.INFO,
                        "[NMSBridgeLoader] Successfully loaded bridge for version: " + rawVersion);
                org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: successfully loaded bridge for %s", rawVersion);
                return (FozminespoofApi) instance;
            } else {
                logger.log(Level.SEVERE,
                        "[NMSBridgeLoader] Class " + className + " does not implement FozminespoofApi");
                org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: class %s does not implement FozminespoofApi", className);
                return null;
            }
            ...
            org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: class not found: %s", className);
            return null;

        } catch (NoSuchMethodException e) {
            logger.log(Level.SEVERE,
                    "[NMSBridgeLoader] No default constructor found in " + className);
            org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: no default constructor in %s", className);
            return null;

        } catch (Exception e) {
            logger.log(Level.SEVERE,
                    "[NMSBridgeLoader] Reflection error while loading bridge: " + e.getMessage(), e);
            org.phantam.fozminespoofapi.utils.DebugLogger.log(logger, "NMSBridgeLoader: reflection error: %s", e.getMessage());
            return null;
        }
    }
}